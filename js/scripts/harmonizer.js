/* 
  ...
  和谐排列脚本
  版本: 2.4
  原作者: Alexander Ladygin, 汉化: AI助手
  描述: 将选中的对象高效排列在指定尺寸的矩形中
...
var panel = win.add('panel', undefined, '和谐排列设置');
...
var captionGutterX = groupGutterX.add('statictext', undefined, '水平间距:'),
    ...
    positionX = groupGutterX.add('dropdownlist', [0, 0, 80, 25], ['左', '中', '右']);
...
var captionGutterY = groupGutterY.add('statictext', undefined, '垂直间距:'),
    ...
    positionY = groupGutterY.add('dropdownlist', [0, 0, 80, 25], ['上', '中', '下']);
    positionY.selection = 1;

var groupCheckbox = panel.add('group');
    groupCheckbox.orientation = 'row';
var toGroupCheckbox = groupCheckbox.add('checkbox', undefined, '分组'),
    randomOrderCheckbox = groupCheckbox.add('checkbox', [0, 0, 100, 20], '随机顺序');
...
with (panel.add('group')) {
    ...
    with (add('group')) {
        ...
        add('statictext', undefined, '排序方式:');
        var reverseOrder = add('checkbox', undefined, '反向排序');
    }
    with (add('group')) {
        orientation = 'row';
        alignChildren = ['fill', 'fill'];
        ...
        sortByS.helpTip = '按大小（宽+高）从大到小排序';
        sortByW.helpTip = '按宽度从大到小排序';
        sortByH.helpTip = '按高度从小到大排序';
    }
}
