...
//打开多页PDF
function openPDF() {
    var win = new Window("dialog", "打开多页PDF");

    var fileGroup = win.add("group"); // this is the group on the left, it holds the File button and the Font label note

    var btnFile = fileGroup.add("button", undefined, "选择文件"); // button to select the PDF to open
    var lblFonts = fileGroup.add("statictext", undefined, "注意\nPDF文件中缺失的字体可能会被替换\n请先将PDF转曲", { multiline: true }); // 
    ...
    var pagesPanel = grpPanel.add("panel", undefined, "页面范围");
    var lblFrom = pagesPanel.add("statictext", undefined, "从:");
    var txtFrom = pagesPanel.add("edittext", undefined, 1);
    var lblTo = pagesPanel.add("statictext", undefined, "到:");
    var txtTo = pagesPanel.add("edittext", undefined, 1);

    var btnGroup = grpPanel.add("group");
    var btnOk = btnGroup.add("button", undefined, "打开");
    var btnCancel = btnGroup.add("button", undefined, "取消");

    var lblStatus = grpRight.add("statictext", undefined, "支持Illustrator CS4及以后版本...");
    ...
    btnFile.onClick = function () {
        ...
        var fileRef = File.openDialog("选择PDF文件...", "*.pdf"); // get the file
    ...
    {							// with all pages on top of each other, create artboards and move each page
        ...
        alert("打开 " + arrPagesInfo.length + " 页，用时 " + time.toFixed(2) + " 秒"); // 35 pages opened in 98-99 seconds
        //$.writeln(time);
